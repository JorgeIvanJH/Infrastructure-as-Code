...
| `setup-internet-control.sh` | nftables and chrony; `spe-internet` and its rule files from [files/internet-control](../../files/internet-control/); the restore unit | a new HTTPS request fails with the internet off and succeeds with it on; the metadata endpoint answers `root` and `terraform` but refuses `speuser` |
