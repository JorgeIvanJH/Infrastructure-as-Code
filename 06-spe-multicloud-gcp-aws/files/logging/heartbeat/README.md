...
flowchart LR
    ...
    subgraph vm["the SPE, at boot"]
        E["169.254.169.254<br>instance metadata endpoint<br>root, terraform, _chrony only"]
...
an SPE can be open or sealed. [spe-internet](../../spe-internet) is the admin command that flips that: `sudo spe-internet off` loads an nftables table, `spe_egress`, whose rules in [spe-internet-disabled.nft](../../spe-internet-disabled.nft) drop every new outbound connection except loopback, replies to the admin's own SSH and RDP sessions, the metadata endpoint, time sync and DHCP; `sudo spe-internet on` deletes the table again. the chosen mode is saved in `/var/lib/spe-internet/mode` and [spe-internet-restore.service](../../spe-internet-restore.service) replays it at boot, so a sealed SPE stays sealed across reboots. a researcher inside the desktop cannot run it, only the admin over SSH can.
...
the probe is itself a connection, so zeek records it: one `network.log` line per heartbeat to `1.1.1.1:443`, `SF` when reachable, `S0` when blocked. that is a known signature, easy to recognise or filter, and it doubles as proof in the network audit that the agent is alive.
...
the heartbeat is the only one of the three streams that already knows its `spe_id`. how it is stored and rotated by journald, and how it compares with the two audit streams, is in [TELEMETRY-PIPELINE-GUIDE.md](../TELEMETRY-PIPELINE-GUIDE.md).
